...
**Composition**
exists when a member of a class has a part-of relationship with the class. In a composition relationship, the class manages the existence of the members. To qualify as a
**composition**
...
**Aggregations**
exists when a class has a has-a relationship with the member. In an aggregation relationship, the class does not manage the existence of the members. To qualify as an
**aggregation**
...
**Associations**
are a looser type of relationship, where the class uses-an otherwise unrelated object. To qualify as an
**association**
...
In a
**dependency**
, one class uses another class to perform a task. The dependent class typically is not a member of the class using it, but rather is temporarily created, used, and then destroyed, or passed into a member function from an external source.
...
In a
**container class**
one class provides a container to hold multiple objects of another type. A
**value container**
is a composition that stores copies of the objects it is holding. A
**reference container**
is an aggregation that stores pointers or references to objects that live outside the container.
...
1a) An Animal class that contains an animal type (enum) and name (string). Show Solution

Composition -- The animal type and name don’t have a use outside of the Animal.
1b) A text editor class with a save() function that takes a File object as an argument. The save() function writes the contents of the editor to disk. Show Solution

Dependency -- The text editor class is using the File object for the task of saving to disk.
1c) An Adventurer class that can carry various kinds of Items, such as swords, wands, potions, or spellbooks. These Items can be dropped and picked up by other Adventurers. Show Solution
...
1d) The Player prays at a Shrine (to receive a new power). Show Solution

Dependency -- They player depends on the shrine to receive the power, but does not have any long-term association with it.
1e) A Computer class that contains a CPU class. The CPU can be removed from the Computer and tested on its own. Show Solution

Aggregation -- The computer has a CPU, but does not manage its existence.
1f) A blacksmith has a specific anvil that he uses to do his work. Show Solution
...
composition
[Next lesson 24.1Introduction to inheritance](https://www.learncpp.com/cpp-tutorial/introduction-to-inheritance/)
Next lesson
24.1
Introduction to inheritance
[Back to table of contents](/)
[Previous lesson 23.7std::initializer_list](https://www.learncpp.com/cpp-tutorial/stdinitializer_list/)
Previous lesson
23.7
