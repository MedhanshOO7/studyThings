...
- Note that all of the arithmetic operators are <mark style="background: #CACFD9A6;">binary operators</mark> -- meaning they take two operands -- one on each side of the operator. 
...
private:
	int m_cents {};
//- `{}` means default-initialize. For an `int`, that becomes 0 if constructed without explicit initialization.
public:
	Cents(int cents) : m_cents{ cents } { }
	//this method is actually better than assignment one 
	//When the object is created, `m_cents` is directly given the value of `cents`.
	
	int getCents() const { 
		return m_cents; 
	}// this const means this function promises not to modify the values
};
```
...
#include <iostream> class Fraction { private: int m_numerator { 0 }; int m_denominator { 1 }; public: explicit Fraction(int numerator, int denominator=1) : m_numerator{numerator}, m_denominator{denominator} { } void print() const { std::cout << m_numerator << '/' << m_denominator << '\n'; } }; int main() { Fraction f1 {1, 4}; f1.print(); Fraction f2 {1, 2}; f2.print(); return 0; }
...
[Next lesson 21.3Overloading operators using normal functions](https://www.learncpp.com/cpp-tutorial/overloading-operators-using-normal-functions/)
Next lesson
21.3
Overloading operators using normal functions
[Back to table of contents](/)
[Previous lesson 21.1Introduction to operator overloading](https://www.learncpp.com/cpp-tutorial/introduction-to-operator-overloading/)
Previous lesson
21.1
