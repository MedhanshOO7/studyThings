...
Key insightThe constexpr keyword is used to signal to the compiler and other developers that a function can be used in a constant expression.
...
For advanced readersThere are some other lesser encountered criteria as well. These can be found here.

There are some other lesser encountered criteria as well. These can be found
[here](https://en.cppreference.com/w/cpp/language/constexpr)
here
.
...
`int`
.
Key insightWhen a constexpr function evaluates at runtime, it evaluates just like a normal (non-constexpr) function would. In other words, the constexpr has no effect in this case.
